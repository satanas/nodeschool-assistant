...
const {
  ...
  MEETUP_GROUP_ID,
  MEETUP_URLNAME,
  EVENT_DESCRIPTION_TEMPLATE_PATH
} = process.env;

const meetup_api = require('meetup-node')(MEETUP_API_KEY);

const eventDescriptionTemplate = fs.readFileSync(path.resolve(EVENT_DESCRIPTION_TEMPLATE_PATH), 'utf8');

async function createMeetupEvent(data) {
  ...
  try {
    ...
    const updatedData = _.assign(data, {
      meetupEventURL: response.event_url,
