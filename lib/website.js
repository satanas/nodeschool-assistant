...
async function publish(config, data) {
  ...
  const commands = [
    ...
    `git commit -a -m 'Automatic update: ${CHAPTER_NAME} - ${moment(Date.now()).format('YYYY-MM-DD hh:mmA')}'`,
  ...
  try {
    for (var i in commands) {
      progressIndicator.text = 'Executing: ' + commands[i];
      let { stdout, stderr } = await exec(commands[i]);
      if (stderr.match(/error/g) || stderr.match(/Aborting/g)) {
        progressIndicator.fail();
        throw stderr;
      }
    }
    ...
    if (err.stdout.match(/nothing to commit/g)) {
      progressIndicator.succeed('Nothing to publish');
    } else {
      progressIndicator.fail();
      throw err;
    }
...
async function sync(config) {
  const progressIndicator = ora('Syncing website').start();
  const command = `git pull ${GITHUB_REMOTE} ${GITHUB_BRANCH}`;

  try {
    let { stdout, stderr } = await exec(command);
    if (stderr.match(/error/) || stderr.match(/Aborting/)) {
      progressIndicator.fail();
      throw stderr;
    }

    progressIndicator.succeed('Syncing website');
  } catch(err) {
    progressIndicator.fail();
    throw err;
  }
