...
async function updateData(config, data) {
  ...
  fs.writeFileSync(config.DATA_JSON_PATH, JSON.stringify(dataJson), { encoding: 'UTF8' });
...
async function build(config, data) {
  ...
  const commands = [
    ...
    // Build styles
    `mkdirp ${config.STYLES_OUTPUT_PATH} && stylus ${config.STYLES_INDEX_SRC_PATH} --out ${config.STYLES_OUTPUT_PATH} --include node_modules --include-css`,
    // Copy images
...
async function publish(config, data) {
  ...
  const commands = [
    ...
    `git commit -m 'Automatic update: ${eventName.replace(/'/g, '"')}'`,
    "git push"
  ];

  if (!PUBLISH_WEBSITE || PUBLISH_WEBSITE === undefined || PUBLISH_WEBSITE === null || PUBLISH_WEBSITE !== 'true') {
    progressIndicator.warn('Publishing is disabled. Add PUBLISH_WEBSITE=true to your .env file or publish manually');
    return data;
  }
